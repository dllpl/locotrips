    ...
    <div class="media">
        ...
        <div class="media-body">
            ...
            @if((!Auth::id() or Auth::id() != $row->create_user ) and setting_item('inbox_enable'))
                <br><a style="width:100%;" class="btn bc_start_chat" href="{{route('user.chat',['user_id'=>$row->create_user])}}" ><i class="fa fa-commenting" style="margin: 0px 10px 0px;"></i> Чат с {{$vendor->getDisplayName()}}</a>
            @endif
        </div>
    </div>
</div>
